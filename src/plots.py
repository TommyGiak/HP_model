...
def create_gif(protein):
    ...
    with writer.saving(fig, 'data/evo.gif', 200):
        ...
        for i, structure in enumerate(protein.gif_struct):
            ...
            y = [coord[1] for coord in structure]
            ...
            # 🔹 Aggiungi titolo dinamico
            ax.set_title(f"Evolution step {i + 1}/{len(protein.gif_struct)}", fontsize=10)

            # 🔹 Cattura il frame con il titolo visibile
            fig.canvas.draw()
            writer.grab_frame()

            # Pulisci per il frame successivo
            ax.clear()
